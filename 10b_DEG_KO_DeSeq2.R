...
counts_file <- "ko_counts.tsv"
meta_file   <- "metadata.tsv"
out_dir     <- "DESeq2_KO_results"
...
if (!"KO" %in% colnames(counts)) {
  
  stop(
    " The counts matrix must contain a 'KO' column."
  )
}

if (anyDuplicated(counts$KO) > 0) {
  
  duplicated_ids <- unique(
    counts$KO[duplicated(counts$KO)]
  )
  
  stop(
    " Duplicate KO identifiers: ",
    paste(
...
if (has_batch) {
  ...
  if (nlevels(meta$batch) < 2) {
    ...
    cat(
      "⚠️ The batch column has only one level — ",
...
if (is.null(dds)) {
  ...
  if (nrow(dds) == 0) {
    ...
    stop(
      " No KO remain after prefiltering."
    )
  }
...
all_volcano_pdf <- file.path(
  plots_dir,
  "Volcano_KO_DESeq2_all_comparisons.pdf"
)

all_volcano_png <- file.path(
  plots_dir,
  "Volcano_KO_DESeq2_all_comparisons.png"
)
...
heatmap_pdf <- file.path(
  plots_dir,
  "Heatmap_significant_KO_DESeq2.pdf"
)

heatmap_png <- file.path(
  plots_dir,
  "Heatmap_significant_KO_DESeq2.png"
)

significant_kos <- all_results %>%
  filter(significant) %>%
  arrange(padj) %>%
  distinct(KO) %>%
  slice_head(
...
if (length(significant_kos) > 1) {
  ...
  if (
    ...
    pheatmap(
      ...
      show_rownames = TRUE,
      fontsize_row = 7,
      main = "Top significant KO – DESeq2"
    )
    ...
    dev.off()
     ...
     cat(
      " Saved heatmap of significant KO.\n"
    )
  ...
  cat(
    " Too few significant KO to generate a heatmap.\n"
  )
}
...
save_plot_if_missing(
  ...
  pdf_file = file.path(
    plots_dir,
    "DESeq2_KO_summary_counts.pdf"
  ),
  png_file = file.path(
    plots_dir,
    "DESeq2_KO_summary_counts.png"
  ),
  width = 8,
  height = 5
)

cat("\n✅ DESeq2 KO analysis finished\n")
cat("📁 Results: ", out_dir, "\n", sep = "")
cat("📄 Combined table: DESeq2_KO_all_comparisons.tsv\n")
cat("📄 Summary: DESeq2_KO_summary_counts.tsv\n")
